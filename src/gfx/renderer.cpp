...
auto Renderer::denoise(vuk::Future _color, vuk::Future _depth, vuk::Future _normal, Camera const& _camera) -> vuk::Future {
	...
	switch (denoiseMode) {
		...
		case DenoiseMode::Bilateral:
			return modules::denoiseBilateral(std::move(_color), std::move(_depth), std::move(_normal), _camera, bilateralParams);
		default:
