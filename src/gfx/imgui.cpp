...
Imgui::Imgui(vuk::Allocator& _allocator) {
	...
	ImGui_ImplGlfw_InitForVulkan(sys::s_glfw->windowHandle(), true);
	...
	// Create font texture
	auto [font, fontFut] = vuk::create_texture(_allocator,
		vuk::Format::eR8G8B8A8Srgb,
		vuk::Extent3D{uint(fontSize.x()), uint(fontSize.y()), 1u},
		fontPixels, false);
	m_font = std::move(font);
	auto comp = vuk::Compiler();
	fontFut.wait(_allocator, comp);
	sys::s_vulkan->context.set_name(m_font, "imgui/font");
...
#include "spv/imgui.frag.spv"
	...
	sys::s_vulkan->context.create_named_pipeline("imgui", imguiPci);
...
auto Imgui::render(vuk::Allocator& _allocator, vuk::Future _target) -> vuk::Future {
	...
	auto imvert = *vuk::allocate_buffer(_allocator, {vuk::MemoryUsage::eCPUtoGPU, vertexSize});
	auto imind  = *vuk::allocate_buffer(_allocator, {vuk::MemoryUsage::eCPUtoGPU, indexSize});
	...
	rg->attach_in("input", std::move(_target));
